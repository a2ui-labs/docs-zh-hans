...
> **说明：** 目前，Gemini Enterprise 中的 A2UI 渲染仅支持通过 Agent-to-Agent（A2A）方式（`a2aAgentDefinition`）注册的自助式代理。部署在 Vertex AI Agent Engine 上、并通过 `adkAgentDefinition` 方式注册的托管代理暂不支持 A2UI 渲染。
...
- **Agent SDK**：[A2UI Python agent SDK](https://github.com/a2ui-project/a2ui/tree/main/agent_sdks/python) 为从代理生成 A2UI 提供了 ADK 扩展
...
[AG-UI](https://ag-ui.com/) 提供协议本身，[CopilotKit](https://www.copilotkit.ai/) 则提供用于构建 agentic 应用的主要全栈框架，并且 **从第一天起就兼容 A2UI**。
...
- **多代理支持**：协调来自多个代理的 UI
- **框架集成**：通过 CopilotKit 支持 React、Vue、Angular 等多种应用形态
...
### AG2：具备原生 A2UI 支持的多代理框架

[AG2](https://ag2.ai/) 是一个广受欢迎的多代理框架，提供先进的代理编排能力。其 [A2UIAgent](https://docs.ag2.ai/latest/docs/user-guide/reference-agents/a2uiagent) 是一个具备原生 A2UI 支持的参考代理，使 AG2 代理能够生成通过 A2A 和 AG-UI 两种方式提供的丰富交互式 UI。

**AG2 如何使用 A2UI：**

A2UIAgent 在 AG2 的 ConversableAgent 基础上扩展了内置的 A2UI 能力——包括提示词工程、带重试的 schema 校验以及 action 处理——因此开发者无需编写自定义渲染代码，就能为其代理加入生成式 UI。

- **校验后的输出**：内置 schema 校验与重试机制，确保 A2UI 生成结果可靠
- **双传输方式**：通过 A2A（JSON-RPC）和 AG-UI（SSE）提供同一份 UI
- **跨平台**：一个代理即可服务 Web、桌面端和移动端客户端
- **自定义目录**：用领域专属组件扩展组件目录

> "A2UIAgent 把 A2UI 协议带入了 AG2，让代理能够以动态、丰富且可交互的 UI 进行表达。可靠、与客户端无关的渲染方式意味着我们的开发者可以把更少时间花在集成上，把更多时间花在打造优秀体验上。"
>
> **— Mark Sze**, AG2 创始工程师
...
- [技术深度解析](https://docs.ag2.ai/latest/docs/blog/2026/03/20/AG2-A2UI/) —— 用 AG2 构建 A2UI 代理
...
**正在生产环境中使用 A2UI？** 欢迎在 [GitHub Discussions](https://github.com/a2ui-project/a2ui/discussions) 分享你的故事。
