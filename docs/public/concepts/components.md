...
为了帮助开发者快速上手，A2UI 团队维护了 [基础目录](../specification/v0_9_1/catalogs/basic/catalog.json)。

这是一个预定义的目录文件，包含一组基础的通用组件（Button、Input、Card）。它并不是某种特殊的目录“类型”；它只是我们已经编写好、并且配有开源渲染器的一个目录版本。

完整的组件画廊和示例，请参见 [组件参考](../reference/components.md)。
...
=== "v0.9 and later"
    ...
    1. LLM 通过 `createSurface` 创建 surface（指定 catalog；在 v1.0 中，还可以同时包含初始数据模型和组件）
