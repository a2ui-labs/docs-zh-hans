...
Renderer 会在发给 agent 的消息 **metadata** 中包含 `a2uiClientCapabilities` 对象（例如 A2A `Message` 的 `metadata` 字段）。
...
为在多 agent 架构中处理路由，orchestrator 必须维护每个 `surfaceId` 到其所属 sub-agent 的映射。官方 Python SDK 提供了 [`A2uiSubagentMap`](../../../agent_sdks/python/a2ui_agent/src/a2ui/adk/orchestration/a2ui_subagent_map.py) 工具类，用于安全地管理这份映射。

完整的可运行示例请参见 [orchestrator agent executor](../../../samples/community/agent/adk/orchestrator/orchestrator_agent_executor.py) 示例。

#### 1. 从 Server Event 中映射所有权

当 sub-agent 发出 A2UI 的 `createSurface` 或 `deleteSurface` 消息时，orchestrator 会拦截该消息，并使用 `update_from_server_event` 把所有权记录到 session state 中。
...
当 client renderer 把 A2UI 的 `action` 或 `error` 消息发回时，orchestrator 会使用 `get_subagent_name_for_client_event` 在 session state 中查找 `surfaceId`，并将请求路由到正确的 sub-agent。
...
该模式确保双向通信循环能为每个功能区域保持完整且有状态。

### 通过 Metadata Stripping 防止数据泄漏

在多 agent 环境中，`a2uiClientDataModel` 对象可能包含多个由不同 sub-agent 拥有的 surface 状态。为防止敏感数据泄漏，orchestrator 必须 **strip** data model metadata，只保留目标 sub-agent 所拥有的 surface。

你可以在 outbound interceptor 中使用 `strip_unowned_surfaces_from_data_model` 方法，就地修改 data model 字典。
...
通过剥离 metadata，orchestrator 可以确保每个 sub-agent 只收到自己有权查看的 data model 部分。

WARNING: **安全风险：状态抓取**：如果 orchestrator 未剥离 `a2uiClientDataModel`，恶意或遭入侵的 sub-agent 可能会读取其他 active surface 的状态。例如，如果 orchestrator 泄漏了整个多 surface data model，一个天气 sub-agent 就可能抓取银行 surface 的敏感数据。剥离是多 agent 系统中的强制安全要求。
