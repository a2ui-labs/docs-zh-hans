...
| 错误 | 原因 | 解决方案 |
| ---------------------- | -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| surface 已存在 | `surfaceId` 已被占用 | 确保 `surfaceId` 在该 renderer 的生命周期内全局唯一。如果使用带有 sub-agent 的 orchestrator，orchestrator 有责任按需管理 surface ID 以避免冲突。 |
| 未找到 surface      | `surfaceId` 不存在             | 确保 `surfaceId` 与已创建的 surface 一致。在 v0.8 中，surface 是隐式创建的，但 v0.9 及以上版本需要显式调用 `createSurface`。 |
