...
1.  **[Sandbox Proxy (`sandbox.html`)](https://github.com/google/A2UI/blob/main/samples/client/shared/mcp_apps_inner_iframe/sandbox.html)**：从同源提供的中间 `iframe`。它在保持结构化 JSON-RPC 通道的同时，将原始 DOM 注入与主应用隔离。
...
MCP server 使用 SSE transport 启动在 `http://localhost:8000`。
...
> ⚠️ **必须使用 `--include=dev`**：Angular CLI（`@angular/cli`）是 dev dependency。如果不使用 `--include=dev`，`ng serve` 不可用。
>
> ⚠️ **`build:renderer` 和 `build:sandbox` 都是必需的**：`build:renderer` 会编译 Angular app 依赖的 A2UI renderer package。`build:sandbox` 会把 sandbox proxy 打包进 Angular 项目的 public assets。缺少任一步，app 都无法工作。
...
| Python version error on `contact_lookup` agent | 安装 Python 3.13+（该示例的 `pyproject.toml` 要求）                                             |
| `npm run build:renderer` fails                 | 确保已先在 `samples/client/lit/` 中运行 `npm install`                                           |
| Angular client shows blank page                | 确保在 `npm start` 前运行了 `npm run build:sandbox`                                             |
| MCP app iframe doesn't load                    | 检查 MCP server（端口 8000）和 proxy agent（端口 10006）是否都在运行                            |
| `ng serve` not found                           | 运行 `npm install --include=dev` 安装包含 `@angular/cli` 在内的 dev dependencies                 |
